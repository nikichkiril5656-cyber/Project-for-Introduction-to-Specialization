Хорошо, я внес изменения в ваш код "Квантового Кликера".

Теперь числа, равные или большие `1e30` (1 нониллион), будут отображаться в научной записи (например, `1.23e+30`, `5.67e+150`). Числа меньше этого порога будут использовать стандартную систему сокращений (K, M, B, T, Qa, Qi, Sx, Sp, Oc, No, и т.д.).

**Что изменилось:**

1.  **Изменена функция `format_number` (теперь строка 135):** Добавлена проверка порога `scientific_threshold = 1e30`. Если число больше или равно этому порогу, оно форматируется с помощью `f"{num:.2e}"`. В противном случае, применяется старая логика с суффиксами.
2.  **Расширение массива суффиксов:** Я добавил больше суффиксов в массив `suffixes` внутри функции, чтобы теоретически покрывать числа до `1e303` (до суффикса `C` - Centillion), хотя на практике числа будут переходить к научной записи раньше, согласно порогу. Это обеспечивает плавный переход в случае, если порог будет изменён или числа будут расти быстрее.
3.  **Форматирование до порога:** Числа до порога научной записи форматируются с 0, 1 или 2 знаками после запятой в зависимости от их величины, как и раньше, но теперь с расширенным набором суффиксов.

Вот полный обновленный файл с изменениями:
